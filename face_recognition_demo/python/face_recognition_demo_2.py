...
from utils_2 import crop
from landmarks_detector_2 import LandmarksDetector
from face_detector_2 import FaceDetector
from faces_database_2 import FacesDatabase
from face_identifier_2 import FaceIdentifier
...
class FrameProcessor:
    ...
    def __init__(self, args):
        ...
        self.landmarks_detector.deploy(args.d_lm, self.QUEUE_SIZE_LIMIT)
        self.face_identifier.deploy(args.d_reid, self.QUEUE_SIZE_LIMIT)
    ...
    def process(self, frame):       
        ...
        print("Iniciou landmarks: " + str(datetime.utcnow()))
        landmarks = self.landmarks_detector.infer((frame, rois))
        print("Acabou landmarks: " + str(datetime.utcnow()))
        
        print("Iniciou infer: " + str(datetime.utcnow()))
        face_identities, unknowns = self.face_identifier.infer((frame, rois, landmarks))
        print("Acabou infer: " + str(datetime.utcnow()))
...
def main():
    ...
    threads = []
    while i < limite:
        if threading.active_count() > limite_threads:
            time.sleep(0.3)
            continue
        t = ThreadWithReturnValue(target=main_orig)
        t.start()
        threads.append(t)
        i = i+1
...
# Rotina que aplica a leitura no video
def leitura_video(array_frame_processor):
    global CNT_FRAMES
    global ID_IDENTIFICADO
        
    args = build_argparser().parse_args()

    frame_processor = array_frame_processor[0]
    cap = open_images_capture(args.input, args.loop)
    #frame_processor = FrameProcessor(args)
    #return frame_processor.faces_database.database
    ...
    while True:
        ...
        threads = []
        print(str(datetime.utcnow()) + " INICIOU CHECK")
        i = 0
        lim = len(array_frame_processor)
        while i < lim:
            frame_processor = array_frame_processor[i]
            t = ThreadWithReturnValue(target=frame_processor.process, args=(frame,))
            t.start()
            threads.append(t)         
            i = i +1
        ...
        for t in threads:
            detections = t.join()
        print(str(datetime.utcnow()) + " FINALIZOU CHECK")
